...
public class FpInfoUtil_11 {
    public static Map<String, Object> parseFPInfo(JSONObject fp_Info) {
	throw new BizException("不支持的票种(卷票)");
    }

    public static Map<String, Object> parseFPInfo2(JSONObject fp_Info) {
	...
	if (fpxxs != null && !"".equals(fpxxs)) {
	    ...
	    //
	    String gfmc = "" + fpxxArr[7];
	    fp_map.put("gmf_mc", gfmc);// 购买方名称
	    String gfsbh = "" + fpxxArr[8];
	    fp_map.put("gmf_nsrsbh", gfsbh);// 购买方税号
	    //
	    String sbbh = "" + fpxxArr[9];
	    fp_map.put("sbbh", sbbh);// sbbh
	    String shy = "" + fpxxArr[15];
	    fp_map.put("shy", shy);// shy
	    //
	    // String hjje = getMoney(fpxxArr[18], rules[2]);
	    // fp_map.put("hjje", hjje);// 合计金额
	    // String hjse = getMoney(fpxxArr[14], rules[2]);
	    // fp_map.put("hjse", hjse);// 合计税额
	    String jshj = getMoney(fpxxArr[11], rules[2]);
